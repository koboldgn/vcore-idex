...
function copy_example_cfg_idex_overrides {
    if [ -d "${CONFIG_DIR}" ]; then
        cp "${SRCDIR}/klipper_config/idex-overrides.cfg" "${CONFIG_DIR}/idex-overrides.cfg"
        echo -e "example idex-overrides.cfg copied"
    else
        echo -e "ERROR: ${CONFIG_DIR} not found."
        exit 1
    fi
...
copy_example_cfg
copy_example_cfg_idex_overrides
link_klippy_extras
